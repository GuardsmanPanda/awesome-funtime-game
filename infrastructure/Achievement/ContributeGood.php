...
class ContributeGood {
    ...
    public static function updateAchievementStatus(User $user): void {
        ...
        while ($au->current_score >= $au->next_level_score) {
            ...
            AchievementUtility::createAnnouncement($user->id, "Contributed panoramas voted Good level: " . $au->current_level, self::ACHIEVEMENT_ID);
        }
        $au->save();
    }
    ...
    public static function updateAllRanks(): void {
        DB::update("
            ...
            FROM (
                ...
                WHERE p.added_by_user_id IS NOT NULL AND pr.rating = 5
    ...
    private static function getScore(User $user): int {
        return DB::selectOne("
            ...
            LEFT JOIN panorama p on pr.panorama_id = p.panorama_id
            WHERE p.added_by_user_id = ? AND pr.rating = 5
        ", [$user->id])->count;
    }
}