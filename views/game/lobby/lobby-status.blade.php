@if(!$game->is_queued)
    <div hx-get="/game/{{$game->id}}/lobby-status" hx-target="#game-status" hx-trigger="load delay:10s">{{t('Waiting on')}}
        <span class="text-emerald-400">{{\App\Models\User::find($game->created_by_user_id)->display_name}}</span>
    </div>
